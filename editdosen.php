...
$id = $_GET['id'];
$sql = "SELECT * FROM tb_dosen WHERE id = $id";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($foto)) {
        $sql = "UPDATE tb_dosen SET nama = '$nama', nik = '$nik', email = '$email', jenis_kelamin = '$jenis_kelamin', tanggal_lahir = '$tanggal_lahir', alamat = '$alamat', foto = '$foto' WHERE id = $id";
        move_uploaded_file($tmp_name, $folder . $foto);
    } else {
        $sql = "UPDATE tb_dosen SET nama = '$nama', nik = '$nik', email = '$email', jenis_kelamin = '$jenis_kelamin', tanggal_lahir = '$tanggal_lahir', alamat = '$alamat' WHERE id = $id";
    }
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . "?id=" . $id);?>" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <?php if (!empty($row['foto'])) { ?>
                    <img src="./uploads/<?php echo $row['foto']; ?>" alt="Foto Dosen" width="100">
                <?php } else { ?>
                    -
                <?php } ?>
            </div>
